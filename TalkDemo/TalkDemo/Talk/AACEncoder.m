...
/*
 {
     Float64             mSampleRate;采样率，每秒多少个采样。如： 16000， 44100， 48000
     AudioFormatID       mFormatID; 数据流的格式
     AudioFormatFlags    mFormatFlags;描述AudioBufferList的格式
     UInt32              mBytesPerPacket;每个数据包，包含的字节数
     UInt32              mFramesPerPacket;每个数据包，包含的采样数
     UInt32              mBytesPerFrame;每个数据帧包含的字节数
     UInt32              mChannelsPerFrame;描述音频通道的个数。单声道 1， 双声道为 2， 多声道 为 n
     UInt32              mBitsPerChannel;每个通道的位深 8, 16, 32等
     UInt32              mReserved;
 }
 */
- (void)setUpConverter:(CMSampleBufferRef)sampleBuffer {
    ...
    //设定声道数为1
    outAudioStreamBasicDescription.mChannelsPerFrame = 1;//这里只能是1，否则报错
    //设定采样率为16000
     ...
     */
    OSStatus result = AudioConverterNewSpecific(
